...
tf_keras = tf.keras  # tf-keras 2.15 standalone is incompatible with TF 2.10

from official.modeling import tf_utils
from official.nlp.modeling import layers

# Provide local fallbacks for token-dropping-specific layers that may not
# be present in newer versions of tf-models-official (they were added in
# 2.10.x as part of the token-dropping project).
if not (hasattr(layers, 'TokenImportanceWithMovingAvg') and
        hasattr(layers, 'SelectTopK')):
    import importlib.util as _ilu
    import os as _os
    _repo = _os.path.dirname(_os.path.abspath(__file__))
    _spec = _ilu.spec_from_file_location(
        'local_layers', _os.path.join(_repo, 'local_layers.py'))
    _ll = _ilu.module_from_spec(_spec)
    _spec.loader.exec_module(_ll)
    if not hasattr(layers, 'TokenImportanceWithMovingAvg'):
        layers.TokenImportanceWithMovingAvg = _ll.TokenImportanceWithMovingAvg
    if not hasattr(layers, 'SelectTopK'):
        layers.SelectTopK = _ll.SelectTopK

...
tf_keras = tf.keras  # use tf.keras (standalone tf-keras 2.15 incompatible with TF 2.10)
...
class TokenDropMaskedLMTask(masked_lm.MaskedLMTask):
  ...
  def build_losses(self,
    ...
    with tf.name_scope('MaskedLMTask/losses'):
      ...
      if 'masked_lm_accuracy' in metrics:
        ...
        per_tok_acc = tf.cast(
            tf.equal(mlm_preds, labels['masked_lm_ids']), tf.float32
        ) * lm_label_weights
        mlm_acc = tf.math.divide_no_nan(
            tf.reduce_sum(per_tok_acc), lm_denominator_loss)
        metrics['masked_lm_accuracy'].update_state(mlm_acc)
  ...
  def train_step(self, inputs, model: tf_keras.Model,
    ...
    with tf.GradientTape() as tape:
      ...
      # record_mlm_loss is specific to TokenDropBertEncoder (updates the
      # per-token importance moving average).  Guard defensively so this task
      # class also works with encoders that don't implement the method.
      _enc = getattr(model, 'encoder_network', None)
      if _enc is not None and hasattr(_enc, 'record_mlm_loss'):
        _enc.record_mlm_loss(

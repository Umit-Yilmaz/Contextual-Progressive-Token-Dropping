...
Production fix: replaced `from official.projects.token_dropping import encoder`
(that sub-package is NOT distributed in tf-models-official 2.10) with a direct
import of the local encoder.py that lives in the same directory as this file.
"""
...
# -- Local encoder import (replaces official.projects.token_dropping.encoder) --
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))
if _THIS_DIR not in sys.path:
    sys.path.insert(0, _THIS_DIR)
from encoder import TokenDropBertEncoder  # noqa: E402
...
def get_encoder(encoder_cfg: TokenDropBertEncoderConfig):
  """Instantiates a TokenDropBertEncoder from config.

  Args:
    encoder_cfg: A TokenDropBertEncoderConfig.

  Returns:
    A TokenDropBertEncoder instance.
  """
  return TokenDropBertEncoder(
      vocab_size=encoder_cfg.vocab_size,
      hidden_size=encoder_cfg.hidden_size,
      num_layers=encoder_cfg.num_layers,
      num_attention_heads=encoder_cfg.num_attention_heads,
      # TokenDropBertEncoder.__init__ remaps these legacy kwarg names:
